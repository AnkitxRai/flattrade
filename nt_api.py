...
class FlattradeApi:
    ...
    def call_api_old(self, key: str, data: dict = None):
    ...
    def call_api(self, key: str, data: dict = None):
        ...
        try:
            ...
            return [{"error": f"Failed to load accounts.json: {e}"}]

        results = []

        # 2. Loop through all active accounts
        for acc in accounts:
            if not acc.get("STATUS", False):
                continue
            ...
            if not uid or not jkey:
                results.append({"account": uid, "error": "Missing UID or TOKEN"})
                continue

            # 3. Build payload for this account
            if data is None:
                data = {}
            else:
                data = dict(data)

            # enforce uid + actid per account
            data["uid"] = uid
            data["actid"] = uid
            ...
            try:
                resp = requests.post(url, data=payload, headers=self.headers, timeout=5)
                resp.raise_for_status()
                results.append({"account": uid, "response": resp.json()})

            except Exception as e:
                results.append({"account": uid, "error": str(e)})
        return results
