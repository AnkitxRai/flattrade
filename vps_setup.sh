...
echo "Test run completed. Check $REPO_DIR/flat_vwap.log for output."

# --- 7. Setup cron ---
echo "[7/7] Setting up cron jobs..."
(crontab -l 2>/dev/null; echo "") | crontab - # ensure crontab exists
CRON_ENTRY="CRON_TZ=Asia/Kolkata
45 9 * * 1-5 cd /root/flattrade && nohup python3 flat_vwap_api.py > /root/flattrade/flat_vwap.log 2>&1 &
